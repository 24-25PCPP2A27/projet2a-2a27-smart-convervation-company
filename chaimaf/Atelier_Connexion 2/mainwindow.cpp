    ...
    , ui(new Ui::MainWindow)
{
    ...
    if (tableView) {
        ...
        if (c.createconnect()) {
            ...
            tableView->setItemDelegateForColumn(11, new BackgroundColorDelegate(palette));

            QPalette palette2;
            palette2.setColor(QPalette::Background, QColor("#FF4545"));

            tableView->setItemDelegateForColumn(12, new BackgroundColorDelegate(palette2));
        } else {
    ...
    // PDF connect
    connect(ui->pdf_2, &QPushButton::clicked, this, &MainWindow::exportToPDF);
    // Excel connect
    connect(ui->excel_2, &QPushButton::clicked, this, &MainWindow::exportToExcel);
    // Search connect
...
void MainWindow::sendDataToArduino(const QString &nom, const QString &prenom) {// Ensure the serial connection is open and writable
        QString dataToSend = nom + "," + prenom + "\n"; // Format: "id,name"
        A.write_arduino(dataToSend.toUtf8()); // Send data to Arduino

}
void MainWindow::sendDataToArduinonotfound() {// Ensure the serial connection is open and writable
...
void MainWindow::on_sortComboBox_currentIndexChanged(int index)
{
    ...
    switch (index) {
        case 1: // Sort by Prénom (A-Z)
            queryStr = "SELECT ID_CL, NOM, PRENOM, EMAIL, DATE_INSCRIPTION, AGE, SEXE, TELEPHONE, NB_SERVICE_RECU, DATE_RECEPTION, DATE_CONSERVATION, 'Update' AS Action, 'Delete' AS Action FROM client ORDER BY PRENOM ASC";
            break;
        case 2: // Sort by ID_CL (Ascending)
            queryStr = "SELECT ID_CL, NOM, PRENOM, EMAIL, DATE_INSCRIPTION, AGE, SEXE, TELEPHONE, NB_SERVICE_RECU, DATE_RECEPTION, DATE_CONSERVATION, 'Update' AS Action, 'Delete' AS Action FROM client ORDER BY ID_CL ASC";
            break;
        case 3: // Sort by Date Inscription (Oldest to Newest)
            queryStr = "SELECT ID_CL, NOM, PRENOM, EMAIL, DATE_INSCRIPTION, AGE, SEXE, TELEPHONE, NB_SERVICE_RECU, DATE_RECEPTION, DATE_CONSERVATION, 'Update' AS Action, 'Delete' AS Action FROM client ORDER BY DATE_INSCRIPTION ASC";
            break;
        default:
            queryStr = "SELECT ID_CL, NOM, PRENOM, EMAIL, DATE_INSCRIPTION, AGE, SEXE, TELEPHONE, NB_SERVICE_RECU, DATE_RECEPTION, DATE_CONSERVATION, 'Update' AS Action, 'Delete' AS Action FROM client";
            break;
    }
    ...
    clientModel->setHeaderData(12, Qt::Horizontal, QObject::tr("Action"));
