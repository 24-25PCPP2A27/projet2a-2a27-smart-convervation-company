...
QSqlQueryModel* traitementproduit::afficher() {
    ...
    if (model->lastError().isValid()) {
        qDebug() << "Error retrieving data for TRAITEMENT_DE_PRODUIT:" << model->lastError().text();
    } else {
        qDebug() << "Data successfully retrieved for TRAITEMENT_DE_PRODUIT.";
    }
...
bool traitementproduit::idExists(int id) {
    QSqlQuery query;
    query.prepare("SELECT COUNT(*) FROM TRAITEMENT_DE_PRODUIT WHERE IDTRAIT = :id");
    query.bindValue(":id", id);
...
// Compare a given temperature with the temperature range for the same ID
bool traitementproduit::isTemperatureWithinRange(int id, int tmp) {
    QSqlQuery query;
    query.prepare("SELECT TEMP_MIN, TEMP_MAX FROM TRAITEMENT_DE_PRODUIT WHERE IDTRAIT = :id");
    query.bindValue(":id", id);

    if (query.exec() && query.next()) {
        int tempMin = query.value("TEMP_MIN").toInt();
        int tempMax = query.value("TEMP_MAX").toInt();

        if (tmp >= tempMin && tmp <= tempMax) {
            return true; // The temperature is within the range
        } else {
            return false; // The temperature is out of range
        }
    } else {
        qDebug() << "Error comparing temperature:" << query.lastError().text();
        return false;
    }
}
